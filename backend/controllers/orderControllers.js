...
// Get order detaills => /api/v1/orders/:id
...
export const allOrders = catchasyncErrors(async (req, res, next) => {
  ...
  if (!orders) {
    return next(new ErrorHandler("No Orders not found,404,404"));
  }
  res.status(200).json({
    orders,
  });
...
export const deleteOrder = catchasyncErrors(async (req, res, next) => {
  ...
  if (!order) {
    return next(new ErrorHandler("No Orders not found,404"));
  }
...
export const getSales = catchasyncErrors(async (req, res, next) => {
  ...
  endDate.setUTCHours(23, 59, 59, 99);
